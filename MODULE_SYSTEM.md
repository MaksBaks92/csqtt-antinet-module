...
| **`(репозиторий AntiNet) examples/modulemasterdns/`** | **Реальный модуль** — MasterDnsVPN / StormDNS (DNS-туннель): вендоренный Go-движок в `native/`, апстрим бампается слиянием. | Референс «по-настоящему» |
| **`(репозиторий AntiNet) examples/moduleqwdtt/`** | **Реальный модуль** — qWDTT (WireGuard-over-TURN через VK-звонки): вендоренный `go_client` плюс обвязка netstack и SOCKS. | Референс вендоринга |
| **`(репозиторий AntiNet) examples/moduleopenflux/`** | **Реальный модуль** — OpenFlux (TCP-туннель поверх мессенджеров): выборка файлов из большого апстрима, раскладка `take: "ours"`. | Референс выборочного вендоринга |
...
- **Пишете модуль** → начните с `MODULE_API.md`, скопируйте `examples/moduleecho/` как скелет,
  смотрите на `(репозиторий AntiNet) examples/modulemasterdns/` как на «по-настоящему».
