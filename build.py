...
def rust_dir_of(m):
    """Каталог Cargo.toml rust-движка, если module.json: build.rustDir задан."""
    rel = (m.get("descriptor") or {}).get("build", {}).get("rustDir")
    if not rel:
        return None
    return REPO / m["root"] / rel


def rust_cdylib_name(goos):
    if goos == "windows":
        return "csqtt_engine.dll"
    if goos == "darwin":
        return "libcsqtt_engine.dylib"
    return "libcsqtt_engine.so"


def cargo_build_lib(rust_dir, extra_env=None, target=None):
    cargo = shutil.which("cargo")
    if not cargo:
        err("cargo не найден — CSQTT-движок собирается Rust'ом (https://rustup.rs)")
        return False
    ...
    info(f"[rust] {' '.join(cmd)}  cwd={rust_dir}")
    r = subprocess.run(cmd, cwd=str(rust_dir), env=env)
    if r.returncode != 0:
        err(f"[rust] cargo build упал (код {r.returncode})")
        return False
    return True


def rust_cdylib_path(rust_dir, goos, target=None):
    name = rust_cdylib_name(goos)
    if target:
        return rust_dir / "target" / target / "release" / name
    return rust_dir / "target" / "release" / name
...
def check_toolchain(targets, mod=None, abis="arm64-v8a", api=26):
    ...
    if rust_dir_of(MODULES.get(mod) or {}) if mod else None:
        cargo = shutil.which("cargo")
        if not cargo:
            problems.append(("Rust/cargo (CSQTT engine)", "https://rustup.rs"))
        else:
            ok(f"cargo: {cargo}")
...
def build_desktop(go, mod, goos, goarch):
    ...
    rust = rust_dir_of(m)
    cgo = "0"
    if rust is not None and goos != "windows":
        # Unix: cgo нужен, чтобы отдать rust-движку protect-callback (dlopen + //export).
        # Windows грузит csqtt_engine.dll через syscall.NewCallback, CGO_ENABLED=0.
        cgo = "1"
    ...
    if rust is not None:
        if not cargo_build_lib(rust):
            return False
        art = rust_cdylib_path(rust, goos)
        if not art.is_file():
            err(f"[rust] нет артефакта {art}")
            return False
        shutil.copy2(str(art), str(out.parent / art.name))
        ok(f"[{mod}/{goos}_{goarch}] rust engine → {out.parent / art.name}")
...
def build_android(mod, abis, api):
    ...
    for abi_dir in sorted(dist_root.iterdir()) if dist_root.is_dir() else []:
        ...
        if rust_dir_of(m) is not None:
            if not build_rust_android_abi(m, abi_dir.name, api):
                return False
    return True


def build_rust_android_abi(m, abi, api):
    """Собирает rust cdylib под Android-ABI и кладёт libcsqtt_engine.so рядом с helper."""
    rust = rust_dir_of(m)
    target = RUST_ANDROID_TARGETS.get(abi)
    if rust is None or not target:
        return True
    ...
    try:
        ...
        spec.loader.exec_module(bm)
        ndk = bm.find_ndk()
        nbin = bm.ndk_bin(ndk) if ndk else None
        goarch, goarm, prefix = bm.ABI_MAP[abi]
        _ = goarch, goarm
        clang = bm.clang_for(nbin, prefix, api) if nbin else None
    except Exception as e:  # noqa: BLE001
        err(f"[rust/{abi}] не удалось найти NDK clang: {e}")
        return False
    if not clang or not Path(clang).exists():
        err(f"[rust/{abi}] нет NDK clang для {target}")
        return False
    ar = nbin / ("llvm-ar.exe" if IS_WIN else "llvm-ar")
    linker_env = f"CARGO_TARGET_{target.upper().replace('-', '_')}_LINKER"
    extra = {
        "CC": str(clang),
        "AR": str(ar),
        linker_env: str(clang),
    }
    if not cargo_build_lib(rust, extra_env=extra, target=target):
        return False
    art = rust_cdylib_path(rust, "linux", target=target)
    if not art.is_file():
        err(f"[rust/{abi}] нет артефакта {art}")
        return False
    dest = REPO / m["root"] / "dist" / "android" / abi / "libcsqtt_engine.so"
    shutil.copy2(str(art), str(dest))
    ok(f"[rust/{abi}] engine → {dest}")
    return True
