...
pub(crate) static PACKET_PORT: AtomicU16 = AtomicU16::new(0);
pub(crate) static TUN_IP: Mutex<String> = Mutex::new(String::new());
...
pub(crate) fn build_runtime(default_worker_threads: usize) -> Result<tokio::runtime::Runtime> {
...
pub(crate) fn runtime_worker_threads(requested_workers: usize) -> usize {
...
pub async fn run(arguments: Arguments) -> Result<()> {
    ...
    *ENGINE_CANCEL.lock().unwrap_or_else(|p| p.into_inner()) = Some(cancel.clone());
    ...
    *ENGINE_PAUSE.lock().unwrap_or_else(|p| p.into_inner()) = Some(paused.clone());
    ...
    if let Ok(port) = local_port.parse::<u16>() {
        PACKET_PORT.store(port, Ordering::Release);
    }
    ...
    let config_task = tokio::spawn(async move {
        ...
        while let Some(config) = config_rx.recv().await {
            ...
            if let Some(value) = config.strip_prefix("TUNCONF:") {
                ...
                if let Ok(mut slot) = TUN_IP.lock() {
                    *slot = ip.to_string();
                }
                if let Ok(mut slot) = TUN_DNS.lock() {
                    *slot = dns.to_string();
                }
