...
- **Ручной** — поля «VK-хеш 1…4» в настройках модуля (или `hashes=` в ссылке). Авто-режимы эти поля не читают.

В авто-режимах нужен вход в VK через webview AntiNet. Фоновый пинг без токена/хешей пропускается (`pingNeedsConsent` + `canping`).

Сервер CSQTT менять не нужно: модуль говорит с ним тем же GETCONF/TUNCONF, что и приложение. По умолчанию в GETCONF уходит `DEVICE_ID` хоста AntiNet (HWID, не `ANDROID_ID` приложения CSQTT). Если пароль уже привязан к приложению — в панели сервера «Отвязать», либо вставьте Device ID из лога CSQTT в настройку модуля.
